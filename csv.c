...
static bool
initialize_row_range (struct csv_s *csv_ctx, size_t start_inc, size_t end_exc)
{
...
static bool
guard_csv_ctx_exists (struct csv_s *csv_ctx)
{
  if (!csv_ctx)
    {
...
static bool
guard_csv_ctx_is_init (struct csv_s *csv_ctx)
{
  if (!guard_csv_ctx_exists (csv_ctx))
    return false;

  if (!(csv_ctx->keys != NULL && csv_ctx->col_dt != NULL
        && csv_ctx->data != NULL && csv_ctx->row_capacity > 0))
    {
...
bool
csv_init (struct csv_s *csv_ctx, const char **keys,
...
static bool
double_csv_row_capacity (struct csv_s *csv_ctx)
{
  ...
  union csv_dt_u **realloc_ret
      = realloc (csv_ctx->data, csv_ctx->row_capacity);
  if (realloc_ret == NULL)
    {
      csv_error (
          "Error doubling csv context data capacity, realloc returned null\n");
      return false;
    }
...
bool
csv_push (struct csv_s *csv_ctx, const union csv_dt_u *row)
{
  if (!guard_csv_ctx_is_init (csv_ctx))
    return false;
  ...
  memcpy (csv_ctx->data[csv_ctx->nrows], row,
          csv_ctx->ncols * sizeof (union csv_dt_u));
  csv_ctx->nrows++;
...
static bool
write_data_to_fp (FILE *fp, union csv_dt_u data, enum csv_dt_e data_type)
{
  switch (data_type)
    {
    case INT:
...
bool
csv_save (struct csv_s *csv_ctx, const char *filename)
{
  if (!guard_csv_ctx_is_init (csv_ctx))
    return false;
  ...
  for (int i = 0; i < csv_ctx->nrows; ++i)
    {
      for (int k = 0; k < csv_ctx->ncols - 1; ++k)
        {
          if (!write_data_to_fp (fp, csv_ctx->data[i][k], csv_ctx->col_dt[k]))
            return false;
          fprintf (fp, ",");
        }
      if (!write_data_to_fp (fp, csv_ctx->data[i][csv_ctx->ncols - 1],
