...
static int64_t
rand_i64_range (int64_t low, int64_t high)
{
  assert (low < high);
...
static double
rand_f64 (double low, double high)
{
  assert (low < high);
...
static void
init_rand_str (char *str, size_t len)
{
  const char chars[]
...
static int
init_rand_str_data (char **data, size_t len)
{
  assert (data);
  for (int i = 0; i < len; ++i)
    {
...
static void
init_rand_i64_data (int64_t *data, size_t len)
{
  assert (data);
  for (int i = 0; i < len; ++i)
...
static void
init_rand_u64_data (uint64_t *data, size_t len)
{
  assert (data);
  for (int i = 0; i < len; ++i)
...
static void
init_rand_f64_data (double *data, size_t len)
{
  assert (data);
  for (int i = 0; i < len; ++i)
